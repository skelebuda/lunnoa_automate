...
const Onborda: React.FC<OnbordaProps> = ({
  ...
  shadowRgb = '0, 0, 0',
  shadowOpacity = '0.2',
  cardTransition = { ease: 'easeOut', duration: 0.3 },
  ...
  const currentTourSteps = useMemo(() => {
    const current = tours.find((tour) => tour.tour === currentTour)?.steps;
    return current;
  }, [currentTour, tours]);
  ...
  }, [currentStep, currentTourSteps, isOnbordaVisible, updatePointerPosition]);
  ...
  return (
    ...
    >
      ...
      <div data-name="onborda-site" className="block w-full h-dvh">
        {children}
      </div>
