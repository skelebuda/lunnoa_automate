...
export class ReplyToThread extends Action {
  ...
  app: Gmail;
  id() {
    return 'gmail_action_reply-to-thread';
  }
  name() {
    return 'Reply to Email Thread';
  }
  description() {
    return 'Reply to an existing email thread using Gmail';
  }
  aiSchema() {
    return z.object({
      threadId: z
...
type ConfigValue = z.infer<ReturnType<ReplyToThread['aiSchema']>>;
