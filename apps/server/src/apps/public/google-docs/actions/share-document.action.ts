...
export class ShareDocument extends Action {
  ...
  app: GoogleDocs;
  id() {
    return 'google-docs_action_share-document';
  }
  name() {
    return 'Share Document';
  }
  description() {
    return `Share a document that ${ServerConfig.PLATFORM_NAME} has created.`;
  }
  aiSchema() {
    return z.object({
...
type ConfigValue = z.infer<ReturnType<ShareDocument['aiSchema']>>;
